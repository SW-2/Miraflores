    ...
    public ArrayList<Curso> buscarPorSeccion(String s){
        ...
        try{
            if(!this.ss.isOpen())
                this.ss = HibernateUtil.getSessionFactory().openSession();
            Transaction tx = this.ss.beginTransaction();
            ret = (ArrayList<Curso>) this.ss.createQuery("from Clases.Curso where sec_id="+s+"and cur_paralelo='A'").list();
            tx.commit();
            this.ss.close();
            return ret;
        }catch(Exception e){
    ...
    public ArrayList<Curso> buscarParalelos(String seccion, String curso){
        ...
        try{
            if(!this.ss.isOpen())
                this.ss = HibernateUtil.getSessionFactory().openSession();
            Transaction tx = this.ss.beginTransaction();
            ret = (ArrayList<Curso>) this.ss.createQuery("from Clases.Curso where cur_nombre="+curso+" and sec_id="+seccion).list();
            tx.commit();
            this.ss.close();
            return ret;
        }catch(Exception e){
